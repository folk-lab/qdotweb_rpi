...
##User must define device name here
DEVICE_NAMES = ["Device_1", "Device_2"]

##User must define interface with device here
def user_defined_data_method(nm, cmd):
    if nm==DEVICE_NAMES[0]:
        return "Device 1 data"+cmd
    if nm==DEVICE_NAMES[1]:
        return "Device 2 data"+cmd
...
#    return "Temperature in Celsius : {:.2f} C\nTemperature in Fahrenheit : {:.2f} F\nPressure : {:.2f} kPa ".format(cTemp,fTemp, pressure)
#
#def read_temp_raw():
...
#        return "Temperature in Celsius : {:.2f} C\nTemperature in Fahrenheit : {:.2f} F".format(temp_c,temp_f)
